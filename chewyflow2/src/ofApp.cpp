...
void testApp::setup(){
    ...
    videoInverted 	= new unsigned char[kinect.width*kinect.height*3];
	videoTexture.allocate(kinect.width, kinect.height, GL_RGB);
...
void testApp::drawtheMidi() {
   ...
   // === Strenth
    ...
    if  (midiMessage.control == 0) {
        ...
        velocityMask.setStrength(ofMap(midiMessage.value, 0, 127, 0, 100));
    ...
    // =====  fluid
        ...
        fluid.setDissipation(ofMap(midiMessage.value, 0, 127, 0, 0.05));
    }
    ...
    // =====  particle
        ...
        particleFlow.setSize(ofMap(midiMessage.value, 0, 127, 0, 5));
    }
        ...
        particleFlow.setSizeSpread(ofMap(midiMessage.value, 0, 127, 0, 10));
    }
    
 
    else if  (midiMessage.control == 21) {
        particleFlow.setBirthChance(ofMap(midiMessage.value, 0, 127, 0, 2));
    }
     ...
     // =====  distence and color
        ...
        velocityMask.setSaturation(ofMap(midiMessage.value, 0, 127, 0, 10));
    }
